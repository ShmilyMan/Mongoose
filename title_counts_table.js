...
var countModel = mongoose.model('title_counts', countSchema);

const data = [];      //存放所有的数据
const noRepeatData = [];    // 存放没有重复的title

datahModel.find({},'publishingOrganizationTitle -_id',function (err , docs) {
  if(!err){
    //1.循环遍历，将所有title数据加入到一个数组中(所有的数据，其中包括重复的数据)
    for (let i = 0; i < docs.length; i++) {
      data.push(docs[i].publishingOrganizationTitle);
    }
    ...
    for (let i = 0; i < noRepeatData.length; i++) {
      datahModel.find({publishingOrganizationTitle:noRepeatData[i]},'key -_id',function (err , docs) {
        if(!err) {
          // console.log(docs.length);
          let insertObj = {};
          if (docs == null || docs == undefined || docs.length <= 0) {
            insertObj = {
              publishingOrganizationTitle:noRepeatData[i],
              titleCount:0
            }
          } else {
            insertObj = {
              publishingOrganizationTitle:noRepeatData[i],
              titleCount:docs.length
            }
          }
